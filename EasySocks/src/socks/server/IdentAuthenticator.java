...
public class IdentAuthenticator extends ServerAuthenticatorNone
{
    ...
    private Vector<InetRange> hosts;
    /** Vector of user hashes*/
    private Vector<Hashtable> users;
    private String user;
...
//////////////////
    ...
    {
        int index = 0;
        Enumeration _enum = hosts.elements();
        while (_enum.hasMoreElements())
        {
            InetRange ir = (InetRange) _enum.nextElement();
            if (ir.contains(ip))
            {
                return index;
            }
    ...
    private String userNames(int i)
    {
        ...
        Enumeration _enum = ((Hashtable) users.elementAt(i)).keys();
        if (!_enum.hasMoreElements())
        {
            return "";
        }
        String s = _enum.nextElement().toString();
        while (_enum.hasMoreElements())
        {
            s += "; " + _enum.nextElement();
        }
