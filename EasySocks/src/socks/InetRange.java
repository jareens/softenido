...
public class InetRange implements Cloneable
{
    ...
    public synchronized boolean add(String s)
    {
        ...
        if (s.charAt(s.length() - 1) == '.')
        {
            ...
            entry = new Object[]
                {
                    s, null, new Long(from), new Long(to)
                };
    ...
    public synchronized void add(InetAddress ip)
    {
        ...
        all.addElement(new Object[]
            {
                ip.getHostName(), ip, new Long(from),
                new Long(to)
            });
    }
    ...
    public synchronized void add(InetAddress from, InetAddress to)
    {
        all.addElement(new Object[]
            {
                from.getHostAddress() + ":" + to.getHostAddress(), null, new Long(ip2long(from)),
                new Long(ip2long(to))
            });
    }
    ...
    {
        Enumeration _enum = all.elements();
        while (_enum.hasMoreElements())
        {
            Object[] entry = (Object[]) _enum.nextElement();
            if (s.equals(entry[0]))
            {
...
/////////////////
    ...
    private synchronized boolean contains(long ip)
    {
        ...
        while (_enum.hasMoreElements())
        {
            Object[] obj = (Object[]) _enum.nextElement();
    ...
    {
        Enumeration _enum = end_names.elements();
        while (_enum.hasMoreElements())
        {
            if (host.endsWith((String) _enum.nextElement()))
            {
                return true;
            }
        }
