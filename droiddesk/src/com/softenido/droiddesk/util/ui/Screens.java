...
public class Screens
{
    ...
    public static void softLock(Activity activity, Handler handler)
    {
        softLock(activity.getWindow(), handler);
    }
    public static void softLock(final Window window, final Handler handler)
    {
        final Runnable task = new Runnable()
        {
            ...
            public void run()
            {
                ...
                if(first) handler.postDelayed(this, 321);
                first=false;
            }
        };
        handler.post(task);
